"""
Core 3-stage build system for vectordotdev
"""
...
class CoreBuildSystem:
    ...
    def download_vector(self, version: str) -> bool:
        ...
        try:
            log_message(f"📥 Completely removing existing /vector...")
            ...
            if not (self.vector_dir / "Cargo.toml").exists():
                log_message(f"❌ Invalid Vector source - no Cargo.toml found")
                return False
        ...
        try:
            ...
            with tarfile.open(fileobj=io.BytesIO(response.content), mode='r:gz') as tar:
                ...
                if not extracted_dirs:
                    log_message(f"❌ No vector directory found in tarball")
                    return False
    ...
    def _clean_git_metadata(self):
        ...
        log_message(f"🧹 Removing git and GitHub metadata...")
        git_items = [
            ...
            if '*' in item_pattern:
                ...
                for match in glob.glob(str(self.vector_dir / item_pattern)):
                    ...
                    if match_path.exists():
                        if match_path.is_dir():
                            ...
                            match_path.unlink()
                        log_message(f"  🗑️ Removed {match_path.name}")
            else:
                ...
                if path.exists():
                    if path.is_dir():
                        ...
                        path.unlink()
                    log_message(f"  🗑️ Removed {item_pattern}")
    ...
    def build_vector_core(self, version: str) -> StageResult:
        ...
        success = self.monitor.monitor_intelligent_build(process, "Vector Core", log_file)
    ...
    def build_vector_bindings(self, vector_result: StageResult) -> StageResult:
        ...
        if not vector_result.success:
            return StageResult(
                BuildStage.VECTOR_BINDINGS, False, 0,
        ...
        if not bindings_dir.exists() or not (bindings_dir / "Cargo.toml").exists():
            log_message("ℹ️ Vector-bindings not configured - skipping stage")
            return StageResult(BuildStage.VECTOR_BINDINGS, True, 0)  # Success (not required)
        ...
        log_message(f"🔗 Linking against Vector artifacts in {self.vector_dir}/target/release")
        ...
        with open(log_file, 'w') as f:
            process = subprocess.Popen([
               ...
               env=env, cwd=bindings_dir)
        
        success = self.monitor.monitor_intelligent_build(process, "Vector Bindings", log_file)
        build_time = time.time() - start_time
        ...
        if success:
            ...
            return StageResult(BuildStage.VECTOR_BINDINGS, False, build_time, error_type, error_msg)
    
    def build_python_layer(self, vector_result: StageResult, bindings_result: StageResult = None) -> StageResult:
        ...
        if bindings_result and bindings_result.success:
            bindings_lib_dir = self.project_root / "vector-bindings" / "target" / "debug"
        ...
        success = self.monitor.monitor_intelligent_build(process, "Python Layer", log_file)
        build_time = time.time() - start_time
