...
class DependencyManager:
    ...
    def sync_vector_dependencies(self, version: str) -> bool:
        ...
        try:
            ...
            if not vector_cargo.exists():
                log_message("Vector Cargo.toml not found - skipping dependency sync")
                return True

            with open(vector_cargo) as f:
            ...
            if not workspace_deps:
                log_message("No workspace dependencies found in Vector")
                return True

            # Read vector-bindings Cargo.toml
            bindings_cargo = self.project_root / "vector-bindings" / "Cargo.toml"
            if not bindings_cargo.exists():
                log_message("vector-bindings/Cargo.toml not found - skipping sync")
                return True

            with open(bindings_cargo) as f:
            ...
            log_message(f"Dependency sync failed: {e}")
    ...
    def auto_fix_vector_bindings(self) -> bool:
        ...
        try:
            ...
            if (
                "toml = {" in content
                and "features = [" not in content.split("toml = {")[1].split("}")[0]
            ):
                content = re.sub(
