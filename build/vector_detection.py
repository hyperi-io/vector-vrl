...
class VectorDetector:
    ...
    def __init__(self, project_root: Path, vector_dir: Path):
        """Set up the project root and Vector source directory."""
            ...
            if result.returncode == 0:
                for line in result.stdout.strip().split("\n"):
                    if "refs/tags/" in line:
                        ...
                        if (
                            tag.startswith("v")
                            and not any(m in tag for m in ["-", "rc", "beta"])
                            and tag.count(".") >= 1
                        ):
                            versions.append(tag)
                            if len(versions) >= 5:
    ...
    def detect_vector_build_info(self) -> dict | None:
        ...
        try:
            ...
            with open(self.vector_dir / "Cargo.toml") as f:
            ...
            vector_artifacts = list(deps_dir.glob("*vector*")) + list(
                release_dir.glob("*vector*")
            )
            if not vector_artifacts:
                log_message(
                    f"Vector {detected_version} partially built - missing key artifacts"
                )
                return None
            ...
            log_message(
                f"Found existing Vector {detected_version} build ({build_age_hours:.1f}h old, {len(vector_artifacts)} artifacts)"
            )
            return build_info

        except Exception as e:
            log_message(f"Build detection failed: {e}")
            return None
    ...
    def should_rebuild_vector(self, build_info: dict, requested_version: str) -> bool:
        ...
        if (
            f"v{existing_version}" == requested_version
            or existing_version == requested_version.lstrip("v")
        ):
            log_message(
                f"Vector {existing_version} already built and matches requested {requested_version}"
            )
            ...
            if build_info["build_age_hours"] < 24:  # Less than 24 hours old
                ...
                log_message(
                    f"Vector {existing_version} build is {build_info['build_age_hours']:.1f}h old - rebuilding"
                )
                return True
        else:
            log_message(
                f"Version mismatch: existing {existing_version} != requested {requested_version}"
            )
            return True
