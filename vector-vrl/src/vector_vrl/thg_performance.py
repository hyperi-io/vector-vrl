...
class THGPerformanceAssessor:
    ...
    def assess_pattern_performance(
        self,
        pattern_name: str,
        vrl_code: str,
        test_logs: list[str],
        ...
        estimated_latency = (
            (processing_time / total_events) * 1000 if total_events > 0 else 0
        )  # ms
    ...
    def _calculate_thg_score(self, metrics: THGMetrics) -> float:
        ...
        total_score = (
            throughput_score + latency_score + reliability_score + efficiency_score
        )
    ...
    def _generate_recommendations(
        self, metrics: THGMetrics, vrl_code: str
    ) -> list[str]:
        ...
        if metrics.events_per_second < 100:
            recommendations.append(
                "Low throughput - consider using built-in parsers instead of string operations"
            )

        if metrics.error_rate_percent > 10:
            recommendations.append(
                "High error rate - review VRL null handling and error cases"
            )

        if metrics.latency_p95 > 100:
            recommendations.append(
                "High latency - optimize VRL for sequential processing"
            )

        if "split(" in vrl_code and vrl_code.count("split(") > 3:
            recommendations.append(
                "Multiple splits detected - consider parse_key_value() or parse_json()"
            )

        if "to_string(" in vrl_code and vrl_code.count("to_string(") > 5:
            recommendations.append(
                "Excessive string conversions - cache converted values"
            )

        if metrics.memory_usage_mb > 50:
            recommendations.append(
                "High memory usage - review intermediate variable creation"
            )
    ...
    def benchmark_multiple_patterns(
        self, pattern_configs: list[dict]
    ) -> list[THGResult]:
    ...
    def _print_benchmark_summary(self, results: list[THGResult]):
        ...
        for i, result in enumerate(sorted_results, 1):
            print(
                f"{i:<4} {result.pattern_name[:24]:<25} "
        ...
        print(
            f"  Top Performance: {max(throughputs):.1f} eps ({sorted_results[0].pattern_name})"
        )
        ...
        print(f"  Needs Work (<100 eps): {needs_work}")


def quick_thg_assessment(
    vrl_code: str, test_logs: list[str], pattern_name: str = "test"
) -> float:
    ...
    result = assessor.assess_pattern_performance(
        pattern_name, vrl_code, test_logs, iterations=20
    )

    print(
        f"Quick THG: {result.thg_score:.1f} ({result.performance_grade}) - {result.metrics.events_per_second:.1f} eps"
    )
