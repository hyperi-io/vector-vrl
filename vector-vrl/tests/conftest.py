...
_SUITE_LABEL = "io.hyperi.test.suite=vector_vrl"
...
def _run_containerized(
    ...
    relative paths to vector. The image's ENTRYPOINT is already the `vector`
    binary, so `args` follows the image directly - prefixing another literal
...
def vector_runner(
    ...
    pytest.skip(
        "no local vector binary and no docker - install docker to run this test"
    )
