...
def audit_test_file(file_path: Path) -> dict[str, list[str]]:
    ...
    for i, line in enumerate(lines, 1):
        ...
        if re.search(
            r'("""|\'\'\').*?(parse_json|parse_syslog|parse_regex|to_int|split|contains)',
            line_content,
        ):
        ...
        if re.search(
            r"(apache_logs|nginx_logs|k8s_logs|json_logs|syslog_logs)\s*=\s*\[",
            line_content,
        ):
...
def fix_test_file(file_path: Path, dry_run: bool = True) -> list[str]:
    ...
    vrl_blocks = re.findall(
        r'(vrl.*?=.*?["\'\"\'\"])(.*?)([\"\'\"\'\"])', content, re.DOTALL
    )
    for i, (prefix, vrl_content, suffix) in enumerate(vrl_blocks):
        if any(
            func in vrl_content
            for func in ["parse_json", "parse_syslog", "parse_regex"]
        ):
            ...
            if not dry_run:
                # Create VRL file
                vrl_path = (
                    file_path.parent / "test_data" / file_path.stem / vrl_file_name
                )
    ...
    data_assignments = re.findall(
        r"(\w+_(?:logs|data))\s*=\s*\[(.*?)\]", content, re.DOTALL
    )
...
def main():
    ...
    print("Auditing vector_vrl test suite for hardcoded content...")
    print("=" * 60)
