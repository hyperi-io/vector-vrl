...
Demonstrates the enhanced vectordotdev THG capabilities
"""
...
def demo_quick_thg_assessment():
    ...
    try:
        score = vectordotdev.quick_thg_assessment(vrl_code, test_logs, "json_parser_demo")
...
def demo_detailed_performance_assessment():
    ...
    try:
        assessment = vectordotdev.assess_vrl_performance(apache_vrl, apache_logs, "apache_access_logs")
...
def demo_vector_pipeline_execution():
    ...
    print("\n⚙️ DEMO 3: Vector Pipeline Execution")
    print("=" * 50)
    ...
    try:
        result = vectordotdev.execute_vector_pipeline(vector_config, key_value_logs)
...
def demo_comparative_benchmark():
    ...
    try:
        assessor = vectordotdev.THGPerformanceAssessor()
...
def main():
    ...
    print("🔥 vectordotdev THG Performance Assessment Demo")
    print("=" * 60)
    print(f"📦 Package Version: {vectordotdev.__version__}")
    print(f"🔗 Bindings Available: {vectordotdev.get_bindings_info()}")
    print()
    ...
    if successful == total:
        ...
        print("⚠️ Some demos failed - check Vector installation and dependencies")
