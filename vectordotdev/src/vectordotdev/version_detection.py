...
class VectorVersionDetector:
    ...
    def fetch_vector_releases(self) -> List[Dict[str, Any]]:
        ...
        try:
            print(f"🔍 Fetching Vector releases from GitHub API...")
            ...
            stable_releases = [
                ...
                if not release.get('prerelease', True) and not release.get('draft', True)
            ]
    ...
    def get_compatible_versions(self, count: Optional[int] = None) -> List[str]:
        ...
        for release in releases:
            ...
            if tag.startswith('v') and not any(x in tag.lower() for x in ['dev', 'rc', 'alpha', 'beta']):
    ...
    def _is_semantic_version(self, version: str) -> bool:
        ...
        try:
            ...
            return (len(parts) >= 2 and 
                   all(part.isdigit() for part in parts[:3]) and
                   int(parts[0]) >= 0 and int(parts[1]) >= 0)
    ...
    def get_latest_compatible_version(self) -> Optional[str]:
        """Get the latest compatible Vector version"""
        
        versions = self.get_compatible_versions(count=1)
        return versions[0] if versions else None
    
    def detect_current_project_version(self) -> Optional[str]:
        ...
        toml_files = ['../vectordotdev/Cargo.toml', './Cargo.toml']
...
def main():
    ...
    parser.add_argument('--current', action='store_true', help='Detect current project version')
