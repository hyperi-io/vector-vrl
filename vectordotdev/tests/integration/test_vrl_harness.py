...
def _run_vector_unit_tests(vector_runner, tmp_path: Path, vrl_source: str, cases: list[tuple[str, str, str]]) -> int:
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            ".level = upcase(.level)\n.processed = true",
            [
                ("upcase_info", '{"level": "info", "message": "test"}', ".processed == true"),
                ("upcase_error", '{"level": "error", "message": "error"}', ".processed == true"),
            ],
            id="basic_transform",
        ),
        pytest.param(
            ...
            [
                ("user_123", '{"user": "123"}', "is_integer(.user_id) && .user_id == 123"),
                ("user_456", '{"user": "456"}', "is_integer(.user_id) && .user_id == 456"),
            ],
            id="field_operations",
        ),
    ],
)
def test_native_execution_matches_subprocess_count(vector_runner, tmp_path, vrl_code, cases):
    """Native execute_vrl() processes the same number of events as `vector test` passes."""
    memory_results = vector_bindings.execute_vrl(vrl_code, [message for _, message, _ in cases])
    subprocess_pass_count = _run_vector_unit_tests(vector_runner, tmp_path, vrl_code, cases)
